...
class FileInputStream {
    ...
    *java:JObject;
    ...
    function close() returns error? {
        ...
        if (obj is error) {
            IOException e = IOException(IOEXCEPTION, message = obj.message(), cause = obj);
            return e;
        }
    }
    ...
    # 
    # + return - The `byte[]|IOException|error` value returning from the Java mapping.
    ...
    # + arg2 - The `int` value required to map with the Java method parameter.
    # + return - The `int|IOException|error` value returning from the Java mapping.
    function readNBytes(byte[] arg0, int arg1, int arg2) returns int|IOException|error {
    ...
    function 'wait() returns error? {
        error|() obj = java_io_FileInputStream_wait(self.jObj);
        if (obj is error) {
...
function newFileInputStream1(File arg0) returns FileInputStream|FileNotFoundException {
    ...
    if (obj is error) {
        FileNotFoundException e = FileNotFoundException(FILENOTFOUNDEXCEPTION, message = obj.message(), cause = obj);
        return e;
    } else { 
...
function FileInputStream_nullInputStream() returns InputStream {
    ...
    InputStream obj = new(externalObj);
    return obj;
}
